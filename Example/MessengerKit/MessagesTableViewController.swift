...
class MessagesTableViewController: UITableViewController {
	...
	let windController = UINavigationController(rootViewController: WindStyleViewController())
    ...
    override func viewDidLoad() {
		...
		//windController.viewControllers.first?.loadViewIfNeeded()
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		case 3:
			splitViewController?.showDetailViewController(windController, sender: nil)
			break
        default:
            break
        }
